...
def runner(args, config):
    ...
    point_nn = Point_NN(input_points=config.npoints, num_stages=args.bftt3d_stages,
                        embed_dim=args.bftt3d_dim, k_neighbors=args.bftt3d_k,
                        alpha=args.bftt3d_alpha, beta=args.bftt3d_beta).cuda()
    point_nn.eval()
    ...
    for corr_id, args.corruption in enumerate(corruptions):
        ...
        for counter in range(n_batches):
            ...
            logits = (-args.bftt3d_gamma * (1 - Sim)).exp() @ label_memory.to(Sim.device)
    ...
    # print_log("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
    print_log(f"Mean classification error source: {sum(error_list_source) / len(error_list_source):.2f}.")
    print_log(f"Mean classification error mixed: {sum(error_list_mixed) / len(error_list_mixed):.2f}.")
