...
class InverseBatchNorm1d(nn.Module):
...
def build_inverse_head(original_head: nn.Sequential):
    layers = []

    for layer in original_head:
        if isinstance(layer, nn.Linear):
            inv_linear = nn.Linear(
                layer.out_features, layer.in_features, bias=layer.bias is not None
            )
            inv_linear.weight.data = layer.weight.data.t().clone()
            if layer.bias is not None:
                # Correct inverse bias: -W.T @ b
                inv_linear.bias.data = -torch.matmul(
                    layer.weight.data.t(), layer.bias.data
                )
            layers.append(inv_linear)
        elif isinstance(layer, nn.BatchNorm1d):
            layers.append(InverseBatchNorm1d(layer))
        elif isinstance(layer, nn.ReLU):
            layers.append(nn.ReLU()) 

    layers = layers[::-1]
    inverse_layers = nn.Sequential(*layers).to(original_head[0].weight.device)
...
def runner(args, config):
    ...
    inverse_class_head = build_inverse_head(source_model.module.class_head)

    class_one_hot = torch.eye(config.model.cls_dim).cuda()
    inverse_emmbd = inverse_class_head(class_one_hot)
    ...
    eval_purge(args, config, logger, source_model, resutl_file_path, inverse_emmbd)
...
def eval_purge(args, config, logger, source_model, resutl_file_path, inverse_emmbd):
    time_list = []
    for args.severity in level:
        for corr_id, args.corruption in enumerate(corruptions):
            ...
            for idx, (data, labels) in enumerate(tta_loader):
                ...
                with torch.no_grad():
                    features = base_model.module.forward_headless(
                        points
                    )
                    features = features / features.norm(dim=-1, keepdim=True)
                    inverse_emmbd = inverse_emmbd / inverse_emmbd.norm(
                        dim=-1, keepdim=True
                    )
                    # # cosine similarity
                    logits = features @ inverse_emmbd.T
                    
                    # equilidean distance
                    # logits = (
                    #     features.unsqueeze(1) - inverse_emmbd.unsqueeze(0)
                    # ).norm(dim=-1)
                    # logits = -logits
