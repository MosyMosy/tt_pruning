...
def run_net(args, config, train_writer=None, val_writer=None):
    config.model.transformer_config.mask_ratio = (
        args.mask_ratio
    )  # overwrite the mask_ratio configuration parameter
    ...
    if not args.partnet_cls:
        ...
        for epoch in range(start_epoch, config.max_epoch + 1):
            ...
            for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
                ...
                points = misc.fps(points, point_all)
                points = points[:, np.random.choice(point_all, npoints, False)]
                ...
                if not args.cyclic:
                    if args.only_cls:  # only supervised classification task
                        ...
                        ret = base_model.module.classification_only(
                            points, only_unmasked=args.only_unmasked
                        )  # todo new flag for 100% or 10% tokens for classification (for ablation study)

                    else:  # joint  training
                        rec_loss, ret, _ = base_model(points)

                else:
                    rec_loss, ret = base_model(
                        points, cyclic=args.cyclic
                    )  # todo for cls loss with 100% and recon loss with 10% tokens
                ...
                if train_writer is not None:
                    ...
                    train_writer.add_scalar(
                        "Loss/Batch/RecLoss", rec_loss.item(), n_itr
                    )
                    train_writer.add_scalar(
                        "Loss/Batch/ClassLoss", class_loss.item(), n_itr
                    )
            ...
            print_log(
                "[Training] EPOCH: %d EpochTime = %.3f (s) Losses = %s, Acc = %s , lr = %.6f"
                % (
                    epoch,
        ...
        for epoch in range(start_epoch, config.max_epoch + 1):
            ...
            for idx, (points, label) in enumerate(train_loader):
                ...
                if args.distributed:
                    loss = dist_utils.reduce_tensor(loss, args)
                    losses.update(
                        [loss.item(), rec_loss.item() * 1000, class_loss.item()]
                    )
                else:
                    losses.update(
                        [loss.item(), rec_loss.item() * 1000, class_loss.item()]
...
def rotate_point_cloud_by_angle(batch_data, rotation_angle):
    ...
    rotated_data[:, 0:3] = torch.matmul(
        shape_pc.reshape((-1, 3)), rotation_matrix.cuda()
    )
    ...
    if not args.partnet_cls:
        ...
        for epoch in range(start_epoch, config.max_epoch + 1):
            ...
            for idx, (taxonomy_ids, model_ids, data) in enumerate(train_dataloader):
                ...
                loss_cls, loss_rot, acc_cls, acc_cls_rot = base_model(
                    points, pts_rot, label, label_rot
                )
                ...
                if train_writer is not None:
                    train_writer.add_scalar("Loss/Batch/Loss", loss.item(), n_itr)
                    train_writer.add_scalar(
                        "Loss/Batch/RecLoss", loss_rot.item(), n_itr
                    )
                    train_writer.add_scalar(
                        "Loss/Batch/ClassLoss", loss_rot.item(), n_itr
                    )
                    train_writer.add_scalar(
                        "Loss/Batch/TrainAcc", acc_cls.item(), n_itr
                    )
                    train_writer.add_scalar(
                        "Loss/Batch/TrainAccRot", acc_cls_rot.item(), n_itr
                    )
                    train_writer.add_scalar(
                        "Loss/Batch/LR", optimizer.param_groups[0]["lr"], n_itr
                    )
            ...
            if train_writer is not None:
                train_writer.add_scalar("Loss/Epoch/Loss_1", losses.avg(0), epoch)
            print_log(
                "[Training] EPOCH: %d EpochTime = %.3f (s) Losses = %s, AccCls = %s, AccSSL = %s , lr = %.6f"
                % (
                    epoch,
                    epoch_end_time - epoch_start_time,
                    ["%.4f" % l for l in losses.avg()],
    ...
    if not args.partnet_cls:
        ...
        with torch.no_grad():
            for idx, (taxonomy_ids, model_ids, data) in enumerate(test_dataloader):
                ...
                acc_cls, acc_cls_rot = base_model.module.classification_only(
                    points, pts_rot, label, label_rot
                )
            ...
            print_log(
                "[Validation] EPOCH: %d  classification Acc = %.4f  rotation pred Acc = %.4f"
                % (epoch, final_acc, final_acc_rot),
                logger=logger,
            )
            ...
            if val_writer is not None:
                val_writer.add_scalar(
                    "Metric/TestAcc/TestAccRot", final_acc, final_acc_rot, epoch
                )
...
def validate(
    ...
    if not args.partnet_cls:
        ...
        with torch.no_grad():
            ...
            final_acc = np.mean(all_acc)
            print_log(
                "[Validation] EPOCH: %d  classification Acc = %.4f"
                % (epoch, final_acc),
                logger=logger,
            )
