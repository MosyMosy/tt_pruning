...
class Attention(nn.Module):
    ...
    def forward_source_prune(self, x, source_stats):
        ...
        x_mask = x_distance <= max(
            misc.best_threshold_model(x_distance), x_distance.min() + 2
...
class Block(nn.Module):
    ...
    def forward_source_prune(self, x, source_stats):
        x = x + self.drop_path(self.attn.forward_source_prune(self.norm1(x), source_stats))
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        return x
...
class TransformerEncoder(nn.Module):
    ...
    def forward_out_intermediate(self, x, pos):
        ...
        for i, block in enumerate(self.blocks):
            x = block.norm1(x + pos)
            intermediate_list.append(x[:,1:,:].flatten(0,1))
    ...
    def forward_source_prune(self, x, pos, source_stats, layer_idx):
        for i, block in enumerate(self.blocks):
            if i in layer_idx:
                x = block.forward_source_prune(x + pos, source_stats)
            else:
                x = block(x + pos)
