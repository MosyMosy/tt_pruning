...
class PointTransformer_prune(nn.Module):
    ...
    def forward_source_prune(self, pts, source_stats=None):
        neighborhood, center = self.group_divider(pts)
        group_input_tokens = self.encoder(neighborhood)  # B G N
        cls_tokens = self.cls_token.expand(group_input_tokens.size(0), -1, -1)
        cls_pos = self.cls_pos.expand(group_input_tokens.size(0), -1, -1)
        ...
        pos = torch.cat((cls_pos, pos), dim=1)
        # transformer
        x = self.blocks.forward_source_prune(x, pos, source_stats)
        x = self.norm(x)
        concat_f = torch.cat([x[:, 0], x[:, 1:].max(1)[0]], dim=-1)
        ret = self.class_head(concat_f)
...
class TransformerEncoder_prune(nn.Module):
    ...
    def forward_source_prune(self, x, pos, source_stats):
        for i, block in enumerate(self.blocks):
